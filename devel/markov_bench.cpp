...
//---------------------------------------------------------------------------
inline double
secondsBetween(const std::chrono::high_resolution_clock::time_point &start,
...
//---------------------------------------------------------------------------
Eigen::MatrixXf buildDesignMatrixNaive(const std::vector<int16_t> &x,
                                       std::size_t M) {
...
//---------------------------------------------------------------------------
Eigen::VectorXf solveCoeffsEigen(const Eigen::MatrixXf &A,
...
//---------------------------------------------------------------------------
// 5) Predict a new sequence using naive float32 arithmetic and round to
// int16_t.
//
...
//---------------------------------------------------------------------------
std::vector<int16_t> predictNaive(const Eigen::VectorXf &coeffs,
  ...
  for (std::size_t i = 0; i < N; ++i) {
    ...
    // clamp into int16 range if you want to be safe, but ignoring extremes
    // here:
    if (r > 32767.f)
      r = 32767.f;
    if (r < -32768.f)
...
int main() {
  ...
  // We'll define "bytes processed" as 2*N for throughput (since int16_t=2
  // bytes).
  ...
  std::cout << "  Build matrix: " << dtBuild << " s, ~" << buildThroughput
            << " MB/s\n";
  std::cout << "  Solve:        " << dtSolve << " s, ~" << solveThroughput
            << " MB/s\n";
  std::cout << "  TOTAL:        " << dtFittingTotal << " s, ~"
            << totalThroughput << " MB/s\n";
  ...
  std::cout << "  Predict: " << dtPredict << " s, ~" << predThroughput
            << " MB/s\n";
