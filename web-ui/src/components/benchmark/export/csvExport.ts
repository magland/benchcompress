...
export const exportToCsv = (data: BenchmarkResult[], selectedDataset: string) => {
  // Convert data to CSV
  ...
  const rows = data.map(row =>
    columns.map(col => {
      ...
      } else if (col.accessorKey === 'encode_time' || col.accessorKey === 'decode_time') {
        return formatNumber(value as number, 4);
      } else if (col.accessorKey === 'original_size' || col.accessorKey === 'compressed_size') {
